...
*---------------------------------------------------------*/
// Hardware
 #define dirPin       2    // stepmoto dir pin
...
// Stepmoto parameter 
 ...
 #define dHomeStep              -200         // 在有HOME SENSOR的情況下回程要多讓馬達多走几步以強制讓馬達啟動HOME動作,做位置校正
 #define motorInterfaceType     1          // stepmoto enable pin (AccelStepper motor type)
                                           // tb6560 use 1

 #define dMaxStep               -3800       // 最大步數:最大的吸氣量
 #define dRateOfInhaleExhale    40         // 吸氣時間比例%  吸氣佔個呼吸週期的時間
                                           // (吸氣時間 / ( 吸氣時間+ 呼氣時間)) * 100
 #define dStepMotoAccel         100000        //  stepmoto 加減速的速度
...
void lcdDisp(int isRun,int BPM,int VOL ,int SPD,int OBPM){
  //lcd.clear();
  lcd.setCursor(8,0); // 設定游標位置在第一行行首
  lcd.print("BPM:  ");   // 每秒次數
  lcd.setCursor(0,1); // 容量 每次吸氣的量百分比
  lcd.print("VOL:  ");
...
int moto_action(int SETST ,int BPM,int VOL,int SPD){
  ...
  static int            overbpm = 0 ;        //1:速度來不及了....  0: BPM正確
  ...
  switch(st){
    ...
    case 4:   // start breathing loop 
        ...
        st++;
    case 5:    //floward  吸氣
      stepper.runToNewPosition (MotoStep*0.88);
      st++;
      //stepper.moveTo(MotoStep);
      //if (stepper.distanceToGo() == 0) { st++; }
      break;
    case 6:    //floward  吸氣
      stepper.setMaxSpeed((SPD*40)*0.2);
      //stepper.setMaxSpeed(1000*50);
      stepper.runToNewPosition (MotoStep);
      
    case 7:    //waiting for MidMillis
      st++; // 不等待直接進入呼氣行程
      if (millis() > MidMillis) { st++;}
      break;
    case 8:    //floward  呼氣
      stepper.setMaxSpeed(800*50);
    ...
    case 9:    //back  呼氣
      ...
      break;
    case 10:    //waiting for EndMillis      
      if (millis() > EndMillis) { st = 4;}
      if (millis() > EndMillis+1)
        overbpm = 1;
      else 
        overbpm = 0;        
      break;     
